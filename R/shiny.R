...
setMethod(f = "shinyReport",
	...
	ui = fluidPage(
		...
		hr(),
		HTML("<p>Generated by <a href='https://bioconductor.org/packages/rGREAT/' target='_blank'>the rGREAT package</a>.</p>"),
		tags$style(
"pre {
	width:800px;
	padding:20px;
...
"
	...
	server = function(input, output, session) {
		...
		observe({
			...
			tb = getEnrichmentTable(object, min_hits = observed_hits_cutoff)
		...
		observeEvent(input$select_term, {
			...
			tb = getRegionGeneAssociations(object, gene_set = term)
			tb = as.data.frame(tb)
