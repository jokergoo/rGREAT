...
<p style='color:red'>Note: from <b>rGREAT</b> 1.99.0, background by `bg`
argument is not supported any more, because <b>GREAT</b> requires a specific format
for <code>gr</code> and <code>bg</code> if both are set, and it uses a completely different method
for the enrichment analysis. <a href="local-GREAT.html#Set_background_regions">You can use local <b>GREAT</b> to integrate background regions.</a></p>
...
```{r, eval = FALSE}
tbl = getEnrichmentTables(job, ontology = c("GO Molecular Function", "BioCyc Pathway"))
...
object which contains the gene-region associations. For those regions that are not associated with any genes under current settings, 
