...
First let's load the **rGREAT** package:
...
We first generate approximately 1000 random regions:
...
The function `great()` performs local GREAT analysis. It has integrated many annotation databases which cover many organisms.
Users simply specify the name of gene set collection (the second argument) and the name of the transcriptome annotation (the third argument).
...
## Manually set gene sets or transcriptome annotations
...
When whole genome is set as background, denote $N_1$ as total number of input regions, $p_1$ as the fraction of genome
that are overlapped to extended TSS for genes in a specific gene set and $K_1$ as the number of regions that overlap to
the gene set associated regions, then the enrichment test is based on the random variable $K_1$ which follows Binomial distribution $K_1 \sim B(p_1, N_1)$.

Similarly, when background regions are set, denote $N_2$ as total number of input regions that overlap to backgroud, $p_2$ as the fraction of background
that are overlapped to extended TSS for genes in a specific gene set and $K_2$ as the number of regions that overlap to
the gene set associated regions and also overlap to background, then the enrichment test is based on the random variable $K_2$ which follows Binomial distribution $K_2 \sim B(p_2, N_2)$.
...
If you want to use the Fisher's exact test-based or Hypergeometric-based method, you can use
the GREAT website or use the Bioconductor package [**LOLA**](https://bioconductor.org/packages/LOLA/).
...
Please the two meta columns are in formats of `CharacterList`
and `IntegerList` because a region may associate to multiple genes.


Of course you can set a specific term by argument `term_id`.
