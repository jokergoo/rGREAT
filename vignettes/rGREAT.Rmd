...
The returned value of `job$getEnrichmentTables()` is a list of data frames in which each one corresponds
...
```{r, eval = FALSE}
tb = job$getGreatTable(ontology = c("GO_Molecular_Function", "BioCyc_Pathway"))
tb = job$getGreatTable(category = c("GO"))
```

All available ontology names for given species can be get by `job$availableOntologies()` 
and all available ontology categories can be get by `job$availableCategories()`. Here you do not
...
Association between genomic regions and genes can be get by `job$plotRegionGeneAssociationGraphs()`.
...
```{r, eval = FALSE}
job$plotRegionGeneAssociationGraphs(type = 1)
```
