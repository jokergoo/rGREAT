...
All available ontology names for given species can be get by `job$availableOntologies()` and all available ontology
categories can be get by `job$availableCategories()`.
...
Here the term ID is from the first column of the data frame which is returned by `job$getGreatTable()`.

```{r, fig.width = 12, fig.height = 4}
par(mfrow = c(1, 3))
job$plotRegionGeneAssociationGraphs(ontology = "GO_Molecular_Function",
    termID = "GO:0004984")
res
```
